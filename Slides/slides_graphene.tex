...
\title[Graphene: Mechanical Properties]{\textbf{Surrogate-based
Assessment of Mechanical Properties of Graphene}}
\thispagestyle{empty}
...
\small{\textbf{Collaborators}: Samit Roy$^{\S}$, Anubhav Roy$^{\S}$}}
\vspace{-1mm}
...
%\Section{surrogate}
\begin{frame}{Polynomial Chaos Surrogate}
...
\small
\vspace{-2.5mm}
\begin{tcolorbox}[width=1.0\textwidth,height=0.35\textwidth,colback=DeepSkyBlue!20,notitle,colframe=DeepSkyBlue!20,colupper=DeepSkyBlue4]
\vspace{-2mm}
\be
\hspace{-2mm}
E(\bm{\xi}) = \sum_{k=0}^{P} c_{k} \Psi_{k}(\bm{\xi}) \nonumber
\ee
\vspace{-2mm}
\scriptsize
\be \bm \xi: f(\bm{\theta})\in \mathcal{U}[-1, 1] \nonumber \ee
\end{tcolorbox}
\end{center}
...
\begin{center}
\vspace{-3.4mm}
\begin{tcolorbox}[width=1.0\textwidth,height=0.41\textwidth,colback=DeepSkyBlue!20,notitle,colframe=DeepSkyBlue!20,colupper=DeepSkyBlue4]
\vspace{-5.5mm}
\small
\be
\begin{aligned}
c_{k} &= \frac{1}{\langle\Psi_{k}^{2}\rangle}\langle\Psi_{k},Q(\bm \xi)\rangle \\
&= \frac{1}{\langle\Psi_{k}^{2}\rangle}\int Q(\bm{\xi})\Psi(\bm{\xi})p(\bm{\xi})d\bm{\xi} \nonumber
\end{aligned}
\ee
\vspace{-1mm}
\end{tcolorbox}
...
\begin{itemize}
\myitem Strain rate: $\dot{\varepsilon}\sim \mathcal{U}[10^9,~10^{12}](\text{s}^{-1}$)
\myitem $\bm{\theta}:~\{\dot{\varepsilon},x_i,y_i\}$
\myitem $x_i\sim \mathcal{U}[0, 30]$(nm), $y_i\sim \mathcal{U}[0, 20]$(nm)
\myitem Single Defect: $i$ = 1
\myitem Two Defects: $i$ = 1,2
\end{itemize}
